...
class TppRekapExport extends DefaultValueBinder implements FromCollection, WithMapping, ShouldAutoSize, WithEvents, WithCustomValueBinder
{
    ...
    public function __construct(?int $bulan = null, ?int $tahun = null, ?User $user = null, ?int $selectedUnitKerjaId = null)
    {
        ...
        $query = Tpp::with(['pegawai.kelasJabatan', 'pegawai.unitKerja', 'unitKerja'])
            ->when($targetUnitKerjaId, function ($inner) use ($targetUnitKerjaId) {
                $inner->where(function ($scoped) use ($targetUnitKerjaId) {
                    $scoped->whereHas('pegawai', fn ($pegawaiQuery) => $pegawaiQuery->where('unit_kerja_id', $targetUnitKerjaId))
    ...
    public function collection()
    {
        ...
        foreach ($this->rows as $tpp) {
            $calc = $this->calculateBreakdowns($tpp);
    ...
    public function map($row): array
    {
        ...
        if (isset($row->_total)) {
            return [
                ...
                $calc = $this->calculateBreakdowns($row);
    ...
    private function calculateBreakdowns(Tpp $tpp): array
    {
